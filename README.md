...
and adds daily transcript summaries of followed YouTube channels. Free to host and, with
the default settings, free to run.
...
- Podcasts lists new YouTube episodes from The Thinking Muslim and 5Pillars.
- Transcript-summary blocks: bullets from the latest captioned video of each channel in
  `transcript_summaries`, built from the video's transcript (see below).
...
- **No key at all**: the page still builds every day, just as a plain list of headlines and
  new episodes instead of a synthesised brief. Transcript summaries need a key.
...
## The transcript summaries

`build.py` (using `nbf.py`) finds the newest matching video on each `transcript_summaries`
channel, pulls its YouTube auto-captions with `yt-dlp` (free, no key), and asks the LLM
for the takeaways as bullets.

Two honest limitations:

- **Caption lag.** YouTube generates auto-captions a few hours to a day after upload, so the
  newest video often has none yet. The code walks back to the newest video that *does*
  have captions, so some days the summary is of yesterday's video until the latest is
  captioned. The block names which video it summarised.
...
the `digest` block (provider, models, title), `transcript_summaries`, and the `sections` list.
...
than stated as fact. Transcript summaries are drawn from each video's own auto-generated
transcript and link to the video.
